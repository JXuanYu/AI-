...
def generate_chain_of_thoughts(data_dict, working_memory):
    """
    Generate tree of thoughts reasoning and prompting by hierarchical rules
    """
    ...
    tree_of_thoughts = {"root": {"thoughts": [], "children": []}}

    for i in range(num_objects):
        obj_traj = np.concatenate([detected_objects[i]["bbox"][:2][None, :], detected_objects[i]["traj"][:6, :]],
                                  axis=0)
        ...
        for t in range(num_future_horizon):
            ...
            size_x, size_y = detected_objects[i]["bbox"][3] * 0.5, detected_objects[i]["bbox"][4] * 0.5
            collision = collision_detection(ego_x, ego_y, CAR_WIDTH / 2., CAR_LENGTH / 2., object_x, object_y, size_x,
                                            size_y)
            if collision:
    ...
    if (object_collisons == 0).all():  # Nothing to care about
        tree_of_thoughts["root"]["thoughts"].append("Notable Objects: None")
        tree_of_thoughts["root"]["thoughts"].append("Potential Effects: None")
    else:
        for i in range(num_objects):
            obj_traj = np.concatenate([detected_objects[i]["bbox"][:2][None, :], detected_objects[i]["traj"][:6, :]],
                                      axis=0)
            for t in range(num_future_horizon):
                if object_collisons[i, t] > 0:
                    ...
                    node = {
                        "thoughts": [
                            f"Notable Objects: {object_name} at ({ox:.2f},{oy:.2f}), moving to ({tx:.2f},{ty:.2f}) at {time} second"],
                        "children": [
                            {"thoughts": [
                                f"Potential Effects: within the safe zone of the ego-vehicle at {time} second"],
                             "children": []}
                        ]
                    }
                    tree_of_thoughts["root"]["children"].append(node)
    ...
    tree_of_thoughts["root"]["thoughts"].append("Driving Plan: " + meta_action)

    def format_tree(node, depth=0):
        message = " " * depth + "-" * depth + " " + "\n".join(node["thoughts"]) + "\n"
        for child in node["children"]:
            message += format_tree(child, depth + 1)
        return message

    cot_message = "*" * 5 + "Tree of Thoughts Reasoning:" + "*" * 5 + "\n"
    cot_message += format_tree(tree_of_thoughts["root"])
